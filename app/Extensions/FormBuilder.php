...
class FormBuilder extends \AdamWathan\Form\FormBuilder
{
    ...
    public function getValueFor($name)
    {
        ...
        $array_based = str_replace(['[', ']'],['.', ''], $name);

        if ($this->hasModelValue($name) && $this->getModelValue($name) !== null) {
            return $this->getModelValue($name);
        }

        if ($this->hasModelValue($array_based)) {
            return $this->getModelValue($array_based);
        }
